...
bool test(uint16_t case_num, uint32_t pmask = 0) {
    ...
    auto e = q.submit([&](handler &cgh) {
      ...
      cgh.parallel_for<class Test>(
          Range, [=](cl::sycl::nd_item<1> ndi) SYCL_ESIMD_KERNEL {
            ...
            uint16_t slm_size = 16 * (VL * VS / 16) * sizeof(T);
            if constexpr (((VL * VS) % 16) > 0)
              slm_size += 16 * sizeof(T);

            slm_init(slm_size * Threads);
            for (int i = 0; i < slm_size; i += 16 * sizeof(T))
              slm_block_store<T, 16>(f + i, simd<T, 16>(new_val + o + i, 1));

            esimd_barrier();

            if constexpr (transpose) {
              auto vals = lsc_slm_load<T, VS, DS, L1H, L3H>(f);
              lsc_flat_store<T, VS>(out + o, vals);
            } else {
              ...
              vals.merge(lsc_slm_load<T, VS, DS, L1H, L3H, VL>(offset, pred),
                         mask);
  ...
  if (!transpose) {
    ...
    for (int i = 0; i < Size; i++) {
      ...
      //             << " vs etalon = 0x" << (uint64_t)e << std::dec <<
      //             std::endl;
      // }
    }
  }
