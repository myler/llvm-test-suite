...
my $feature_folder = "SYCL_ESIMD_EMBARGO";
my $feature_name = "";
my $config_folder = "config_sycl_esimd_embargo";
my $suite_name = "llvm_test_suite_esimd_embargo";
...
sub gen_suite
{
    ...
    if ($feature_folder eq "SYCL_ESIMD_EMBARGO") {
        if ($subsuite ne "") {
            # For subsuite
            ...
            $xml->{files}       = { file => [ { path => 'cmake'}, { path => 'tools'}, { path => 'CMakeLists.txt'}, { path => 'litsupport'}, { path => 'lit.cfg'}, { path => 'lit.site.cfg.in'}, { path => 'SYCL'}, { path => 'SYCL_ESIMD_EMBARGO'},  {path => 'SYCL/include', dst => 'SYCL_ESIMD_EMBARGO/include'}, { path => '$INFO_TDRIVE/ref/lit'}, { path => $config_folder}, { path => '.github/CODEOWNERS'}, { path => 'setenv.list'}]};
        }
    } else {
        if ($subsuite ne "") {
            # For subsuite
